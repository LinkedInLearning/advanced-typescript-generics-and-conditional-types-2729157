...
function getNotificationDetails(notification) {
    switch (notification.type) {
        case "departure":
            return `Gate ${notification.gate} at ${notification.time}`;
        case "arrival":
            return `Terminal ${notification.terminal} at ${notification.time}`;
        case "delay":
            return `New Time: ${notification.newTime}, Reason ${notification.reason}`;
        case "cancellation":
            return `Reason: ${notification.reason}`;
    }
}
console.log("Notification Details");
notifications.forEach((notification) => {
    console.log(`${notification.type} for ${notification.airport.name}: ${getNotificationDetails(notification)}`);
});
